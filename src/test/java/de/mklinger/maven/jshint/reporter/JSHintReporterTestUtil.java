...
class JSHintReporterTestUtil {
	...
	private static JSHint.Hint createHint(final String code, final int line, final int character, final String evidence, final String reason) {
		final JSHint.Hint error = new JSHint.Hint();
		error.id = "(error)";
		error.code = code;
		error.line = line;
	...
	static Results createMultipleFilesFailedResults() {
		...
		}
		{
			...
			final Result result = new Result("/path/to/B.js", 1377309240000L, hints);
			results.add(result);
		}
		{
			final List<Hint> hints = new ArrayList<Hint>();
			...
			final Result result = new Result("/path/to/C.js", 1377309240000L, hints);
			results.add(result);
		}
		return results;
	}
